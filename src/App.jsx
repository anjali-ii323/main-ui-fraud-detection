...
function App() {
  ...
  const [routeTransitionFlash, setRouteTransitionFlash] = useState(false)
  ...
  useEffect(() => {
    setRouteTransitionFlash(true)
    const timer = window.setTimeout(() => setRouteTransitionFlash(false), 130)
    ...
    >
      ...
      <AnimatePresence>
        {routeTransitionFlash ? (
          <motion.div
            key={`route-flash-${location.pathname}`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 0.12 }}
            exit={{ opacity: 0 }}
            transition={{ duration: 0.12, ease: 'easeInOut' }}
            className="pointer-events-none fixed inset-0 z-40 bg-slate-950/60 backdrop-blur-[1px]"
          />
        ) : null}
      </AnimatePresence>
